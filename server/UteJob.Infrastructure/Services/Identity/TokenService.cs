...
namespace UteJob.Infrastructure.Services.Identity
{
    public class TokenService : ITokenService
    {
        ...
        public async Task<Result<TokenResponse>> LoginAsync(TokenRequest model)
        {
            ...
            var response = new TokenResponse { Token = token, RefreshToken = user.RefreshToken, FirstName = user.FirstName, Email = user.Email };
            return await Result<TokenResponse>.SuccessAsync(response);
        }

        private async Task<string> GenerateJwtAsync(UteJobUser user)
        {
            var token = GenerateEncryptedToken(GetSigningCredentials(), await GetClaimsAsync(user));
            return token;
        }

        private async Task<IEnumerable<Claim>> GetClaimsAsync(UteJobUser user)
        {
